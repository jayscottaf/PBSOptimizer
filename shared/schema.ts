...
export const pairings = pgTable(
  ...
  table => ({
    // Nearly every pairings query filters by bid_package_id first.
    bidPackageIdIdx: index('pairings_bid_package_id_idx').on(
      table.bidPackageId
    ),
  })
);
...
export const bidHistory = pgTable(
  ...
  table => ({
    // History matching filters on (base, aircraft); auto-linking and
    // duplicate cleanup scan by linked_pairing_id.
    baseAircraftIdx: index('bid_history_base_aircraft_idx').on(
      table.base,
      table.aircraft
    ),
    linkedPairingIdIdx: index('bid_history_linked_pairing_id_idx').on(
      table.linkedPairingId
    ),
  })
);
...
export const chatHistory = pgTable(
  'chat_history',
  {
    ...
    userId: integer('user_id').references(() => users.id), // nullable: historical rows predate cross-device sync
...
export const userCalendarEvents = pgTable(
  ...
  table => ({
    userIdIdx: index('user_calendar_events_user_id_idx').on(table.userId),
  })
);
