              ...
              try {
                ...
                const extractDatesFromRange = (text: string) => {
                  ...
                  const weekdayTokens = Array.from(
                    t.matchAll(/\b(SU|MO|TU|WE|TH|FR|SA)\b/g)
                  ).map(x => x[1]);
                  ...
                  if (weekdayTokens.length > 0) {
                    console.log(
                      'Returning endpoints only due to weekday qualifiers'
                    );
                    return [start, end];
                  }
                  ...
                  console.log(
                    'Date range results:',
                    out.map(d => d.toLocaleDateString())
                  );
                  return out;
                };

                let possibleStartDates: Date[] = [];
                const cleaned = effectiveDateStr
                  .replace(/EFFECTIVE/g, '')
                  .replace(/ONLY/g, '')
                  .trim();
                ...
                if (possibleStartDates.length === 0) {
                  ...
                  if (single) {
                    possibleStartDates.push(single);
                  }
                }
                ...
                if (possibleStartDates.length <= 1) {
                  ...
                  if (effLine) {
                    console.log(
                      'Fallback using fullText EFFECTIVE line:',
                      effLine
                    );
                    const cleanedEff = effLine
                      .replace(/EFFECTIVE\s*/g, '')
                      .trim();
