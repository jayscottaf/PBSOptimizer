    ...
    req.onupgradeneeded = event => {
      ...
      if (oldVersion < 2) {
        ...
        if (db.objectStoreNames.contains('pairings')) {
          ...
          if (
            pairingsStore &&
            !pairingsStore.indexNames.contains('updatedAt')
          ) {
            pairingsStore.createIndex('updatedAt', 'updatedAt', {
              unique: false,
            });
...
async function put(
  ...
  return new Promise((resolve, reject) => {
    ...
    (tx.objectStore(store) as IDBObjectStore).put({
      key,
      data,
      updatedAt: Date.now(),
    } as PairingCacheRecord);
...
async function get(
  store: 'pairings' | 'stats',
  key: string
): Promise<PairingCacheRecord | undefined> {
  const db = await openDB();
  return new Promise((resolve, reject) => {
...
export async function purgeUserCache(userId: string | number): Promise<void> {
  ...
  try {
    ...
    const userPairingsKeys = pairingsKeys.filter(key =>
      key.startsWith(userPrefix)
    );
    console.log(
      `Found ${userPairingsKeys.length} pairing cache entries to purge`
    );
...
export async function migrateOldCacheFormat(): Promise<void> {
  try {
    ...
    if (info.userCacheStats['no-user'] > 0) {
      console.log(
        `Found ${info.userCacheStats['no-user']} entries without user prefix, migration recommended`
      );
    ...
    console.warn(
      'Cache migration check failed (this is normal for first-time users):',
