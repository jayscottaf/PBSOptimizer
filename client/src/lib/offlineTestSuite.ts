...
import { getCacheInfo, hasFullPairingsCache, cacheKeyForPairings } from './offlineCache';
...
export class OfflineTestSuite {
  ...
  async runAllTests(): Promise<TestSuite[]> {
    ...
    console.log(totalPassed ? '✅ All test suites passed!' : '❌ Some tests failed');
  ...
  private async runCacheTests(): Promise<TestSuite> {
    ...
    results.push(await this.runTest('Cache Info Access', async () => {
    ...
    results.push(await this.runTest('Cache Key Generation', async () => {
      ...
      if (key1 !== key2) throw new Error('Identical filters should generate same key');
      if (key1 === key3) throw new Error('Different filters should generate different keys');
      if (!key1.includes('user:15600:')) throw new Error('User ID not in cache key');
      
      return { key1, key2, key3 };
    }));

    // Test 3: IndexedDB functionality
    results.push(await this.runTest('IndexedDB Functionality', async () => {
      const dbRequest = indexedDB.open('pbs-cache');
      return new Promise((resolve, reject) => {
        dbRequest.onsuccess = () => {
          const db = dbRequest.result;
          const hasRequired = db.objectStoreNames.contains('pairings') && 
  ...
  private async runNetworkTests(): Promise<TestSuite> {
    ...
    results.push(await this.runTest('Health Check Endpoint', async () => {
    ...
    results.push(await this.runTest('Network Status Detection', async () => {
    ...
    results.push(await this.runTest('Retry Mechanism', async () => {
      let attempts = 0;
      ...
      const result = await this.healthChecker.retryWithBackoff(mockOperation, 3, 'mock-test');
      if (result.attempts !== 2) throw new Error('Retry logic incorrect');
      return result;
    }));
  ...
  private async runUITests(): Promise<TestSuite> {
    ...
    results.push(await this.runTest('Service Worker Registration', async () => {
    ...
    // Test 2: PWA manifest
    results.push(await this.runTest('PWA Manifest', async () => {
      const manifestLink = document.querySelector('link[rel="manifest"]') as HTMLLinkElement;
      if (!manifestLink) throw new Error('Manifest link not found');
      
      const response = await fetch(manifestLink.href);
      if (!response.ok) throw new Error('Manifest not accessible');
    ...
    results.push(await this.runTest('Critical DOM Elements', async () => {
      const elements = {
        pairingTable: !!document.querySelector('[data-testid="pairing-table"], table'),
        statsPanel: !!document.querySelector('[data-testid="stats-panel"]') || !!document.querySelector('.text-2xl'),
        networkStatus: !!document.querySelector('[data-testid="network-status"]') || true // Component might not be visible
      };
  ...
  private async runDataIntegrityTests(): Promise<TestSuite> {
    ...
    results.push(await this.runTest('Local Storage Access', async () => {
    ...
    results.push(await this.runTest('Query Client State', async () => {
      // Check if React Query is available
      const hasQueryClient = !!(window as any).queryClient || 
    ...
    results.push(await this.runTest('Critical App State', async () => {
      // Check for essential app data in DOM or storage
      const hasBidPackageData = !!document.querySelector('[data-testid="bid-package"]') ||
  ...
  if (typeof window !== 'undefined') {
    (window as any).offlineTests = {
      runAll: async () => {
        ...
        console.table(results.flatMap(suite => 
      ...
      testUICleanness: () => {
        ...
        const offlineBanners = document.querySelectorAll('.chrome-offline-banner, .browser-offline-bar');
        if (offlineBanners.length > 0) {
          issues.push('Browser offline banners detected');
        }
        
        // Check for WiFi icon presence (now inline)
        const wifiIcon = document.querySelector('svg[class*="h-5 w-5 text-"]') ||
        ...
        const redBanner = Array.from(document.querySelectorAll('*')).find(el => 
        ...
        const uploadButton = document.querySelector('button:has(svg + span[class*="Upload"], [class*="CloudUpload"])');
        const networkStatusContainer = document.querySelector('.relative:has(.w-9.h-9.rounded-lg)');
        if (uploadButton && networkStatusContainer) {
          ...
          if (heightDiff > 5) { // Allow 5px tolerance
